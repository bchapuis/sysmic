...
/**
  ...
  */
abstract class LinearSplitter(M: Int, m: Int) extends Splitter {
  ...
  private[rtree3d] def pickSeeds(entries: List[Entry]): (List[Entry], List[Entry], List[Entry]) = {
    ...
    val hlsz = entries.maxBy(e => e.mbb.z1)
    val lhsz = entries.minBy(e => e.mbb.z2)
    val sepz = (hlsz.mbb.z2 - lhsz.mbb.z1) / (r.z2 - r.z1)
